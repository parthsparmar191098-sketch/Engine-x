...
async function loadMachine(id) {
  ...
  el.machineName.textContent = def.name; el.machineSub.textContent = def.sub; document.title = `${def.name} · Engine-X`;
...
(function trailerMode() {
  ...
  let idx = MACHINES.findIndex(m => m.id === current?.id);
  if (idx < 0) idx = 0;
  let cycleTimer = null;
  let active = true;
  ...
  cycleTimer = setInterval(nextMachine, 7000);
